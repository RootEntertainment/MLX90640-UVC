	...
	} else
	{
		*outstr_ptr++ = 0x01;    	//for Y
		if (jpgenc->img_format == JPG_IMGFMT_YUV422)
			*outstr_ptr++ = 0x21;
		else if (jpgenc->img_format == JPG_IMGFMT_YUV420)
			*outstr_ptr++ = 0x22;
		else
...
/*	
UInt16 getPixYUV(UInt32 x,UInt32 y,uint8_t* temp){
	uint8_t gray;
	uint32_t x1,y1;
	...
	//float u=y-y1*319/31.0;
	//float v=x-x1*239/23.0;
	
	gray=temp[x1*32+y1];
	
	if(y%2){
		return colormap1[gray].v<<8|colormap1[gray].y;
	}
	else{
		return colormap1[gray].u<<8|colormap1[gray].y;
	}
...
void JPG_readsrc_yuv(UInt32 xoffset, UInt32 yoffset, UInt32 yuv_mode, Int16 *mcubuf)
{
	...
	for (i = 0; i < rownum; ++i)
	{
		...
		while (j < 16)
		{
			//buf[j++] = getPixYUV(yoffset + i,xoffset+ex_ptr,jpgenc->src);
			//buf[j++] = getPixYUV(yoffset + i,xoffset+ex_ptr+1,jpgenc->src);
		 
			buf[j++] = jpgenc->src[yoffset + i][xoffset+ex_ptr];
			buf[j++] = jpgenc->src[yoffset + i][xoffset+ex_ptr+1];
...
void JPG_setOutBuff(UInt8 *outbuf){
	jpg_outstream = outbuf;
	jpg_streamfptr = outbuf;
}
